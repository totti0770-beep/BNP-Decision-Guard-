...
> deployment of `9eb03d6` on 2026-09-22 booted with
> `chunks=2760 staleRetrievable=0 staleOrphaned=0` (deployment `598377b9`;
> 2,706 on 2026-09-14) — the corpus has almost quadrupled since go-live, and
> the index is still coherent. Which documents those chunks belong to, and
> whether each passed the governed workflow, is what `GET /documents/inventory`
> reports and what nobody has yet pulled.
...
| Index integrity (advisory lock + UNIQUE constraint, real column-width check) | ✅ | ✅ | ✅ `staleRetrievable=0` on **2,760** chunks — re-measured 2026-09-22 from the post-merge boot log of `9eb03d6` (2,706 on 2026-09-14; 725 at go-live) |
...
| Integration/E2E tests (real HTTP + Postgres, browser smoke) | ✅ 517 API unit + 278 integration against real Postgres+pgvector + 24 web unit (all measured on commit `9eb03d6`) + the browser flow, all gate CI | ✅ | ✅ |
...
| Approved clinical content corpus | 🔴 synthetic | 🟡 **2,760** chunks **indexed** in production (`chunks=2760`, boot log of deployment `598377b9`, 2026-09-22; the go-live table below records 725 on 2026-08-22); whether they were approved through the governed workflow is unaudited — see the runbook row *Real approved clinical corpus* | ✅ |
