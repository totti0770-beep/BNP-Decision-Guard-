...
| **Object storage access** | `storage/storage.service.ts` | Every S3 value resolves through `loadEnv()`, never a call-site fallback. Download links are presigned and expire in **300 seconds**, issued only to `documents:download` holders. The readiness probe reports a missing bucket rather than creating one, so a broken deployment fails its check instead of looking self-healing. `storage.service.spec.ts` asserts all four against a mocked client. |
| **Pre-activation conflict gate with dual-control waiver** | `findings/scan.service.ts`, `findings/conflict-gate.service.ts`, `findings/findings.service.ts`, `approval.service.ts` | `submit-review` runs a deterministic L1 scan (ISMP error-prone abbreviations, trailing-zero and naked-decimal doses, page-count and version-marker mismatches, unambiguous date-order errors, unresolved appendix references, foreign-health-system terminology) *before* the status changes, and `approve()` refuses with 400 while a `BLOCKING` finding stands against the current version, auditing `DOCUMENTS:APPROVE_BLOCKED`. The gate is service-level, not a route guard, because `seed.ts` calls `approve()` with no HTTP request. **Fail-safe by construction:** an extraction failure or timeout becomes a `SCAN_FAILED`/`SCAN_TIMEOUT` finding at `BLOCKING`, so "the scanner did not run" is never indistinguishable from "the scanner found nothing". A blocking waiver needs signatures from two different users covering both `PHARMACIST_REVIEWER` and `CBAHI_QUALITY_OFFICER`, checked on **role membership read from the database** — not the permission (`SUPER_ADMIN` holds all of them and would satisfy both halves alone) and not the JWT (a login-time snapshot). Whoever created the version under review may not sign against it. `test/conflict-detection.e2e-spec.ts` proves each of these over real HTTP, including the lone-`SUPER_ADMIN` refusal, the DB-level absence of an APPROVE row after a block, and that a waiver on v1 does not carry to v2; 23 mutations (drop the gate, widen the query to MAJOR, drop the version predicate, authorise on permissions, count signatures instead of covering roles, read roles from the token, anchor separation of duties on `uploaded_by_id`, rethrow the extraction error, …) each fail the named test. |
